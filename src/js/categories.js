...
function drawPieChart(canvas, breakdown, { highlightIndex = -1 } = {}) {
    if (!canvas) return { slices: [], meta: null };
    const { items, total } = breakdown;
    const setup = prepareOverlayCanvas(canvas);
    if (!setup || !total) return { slices: [], meta: null };
    ...
    items.forEach((item, index) => {
        ...
        ctx.fill();
        slices.push({ startAngle, endAngle, index });
        startAngle = endAngle;
    });
    ...
    if (highlightIndex >= 0 && slices[highlightIndex]) {
    ...
    return { slices, meta: { centerX, centerY, radius } };
}

function drawNotesChart() {
    const result = drawPieChart(notesOverlayCanvas, notesBreakdown, { highlightIndex: activeSegmentIndex });
    notesBreakdownSlices = result.slices;
    notesChartMeta = result.meta;
}

function drawArchivedChart() {
    const result = drawPieChart(archivedOverlayCanvas, archivedBreakdown);
    archivedBreakdownSlices = result.slices;
    archivedChartMeta = result.meta;
}
